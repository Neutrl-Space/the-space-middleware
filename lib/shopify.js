...
function getShopifyClientSecrets() {
  return [
    process.env.SHOPIFY_CLIENT_SECRET,
    process.env.SHOPIFY_PREVIOUS_CLIENT_SECRET,
  ].filter(Boolean);
}
...
// New OAuth requests are always signed with the current secret.
function sign(value) {
  return signWithSecret(
    value,
    requiredEnvironment('SHOPIFY_CLIENT_SECRET')
  );
}
...
function safeEqualHex(left, right) {
  ...
  if (!validHex.test(leftValue) || !validHex.test(rightValue)) {
    return false;
  }
...
// During rotation, callbacks signed with either secret remain valid.
function verifyWithShopifySecrets(value, receivedSignature) {
  return getShopifyClientSecrets().some((secret) =>
    safeEqualHex(
      receivedSignature,
      signWithSecret(value, secret)
    )
  );
}
...
function validateState(state) {
  ...
  return verifyWithShopifySecrets(payload, signature);
}
...
function verifyCallbackHmac(query) {
  ...
  return verifyWithShopifySecrets(message, receivedHmac);
}
